...
class DrugDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        for row in batch:
            ...
            # print(h_graph)
            # DrugData1 = Data(h = h_graph,
            #                  t = t_graph,
            #                  rel = torch.LongTensor([Y]),
            #                  label = torch.FloatTensor([1]))
            # data_list.append(DrugData1)
            #
            # DrugData2 = Data(h = n_graph,
            #                  t = t_graph,
            #                  rel = torch.LongTensor([Y]),
            #                  label = torch.FloatTensor([0]))
            # data_list.append(DrugData2)
...
def load_ddi_dataset(root, batch_size, fold=0):
    ...
    train_loader = DrugDataLoader(train_set, batch_size=batch_size, shuffle=True, drop_last=True)
    val_loader = DrugDataLoader(val_set, batch_size=batch_size)
    test_loader = DrugDataLoader(test_set, batch_size=batch_size)
