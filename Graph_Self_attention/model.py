...
class GraphTransformer(nn.Module):
    def __init__(self, in_size, d_model, num_heads=,
                 ...
                 batch_norm=False, abs_pe=False, abs_pe_dim=,
                 gnn_type="", se="", use_edge_attr=False, num_edge_features=4,
                 use_global_pool=True, global_pool='', **kwargs):
        super().__init__()
        ...
        self.embedding = nn.Linear(in_size,d_model,bias=False)

        self.use_edge_attr = use_edge_attr
        if use_edge_attr:
            edge_dim = kwargs.get('edge_dim', 32)
            self.embedding_edge = nn.Linear(num_edge_features, edge_dim, bias=False)
        else:
